...
import { buildCollectionReportData } from '@/lib/reportData';
...
// Параметри: format (співвідношення), top (глибина топу), download (віддати як файл) — як у стрім-роуті.
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await buildCollectionReportData(prisma, U, id, top);
  if (!data) return new Response('Збір не знайдено', { status: 404 });
  ...
  return new Response(buf, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="collection-${id}-${format}.png"`,
    },
  });
}
